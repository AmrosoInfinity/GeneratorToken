...
def button_handler(update, context):
    ...
    if data in ["grab", "gojek"]:
        # Pastikan hanya di grup (sesuai kebutuhan operasional ojek)
        ...
        # --- EKSEKUSI GRAB ---
        if data == "grab":
            # Memanggil handler yang melakukan Double JWT Generator
        ...
        # --- EKSEKUSI GOJEK ---
            ...
            if check_limit(update, context, tz_name, user_id, user_requests, user_blocked, user_timezone):
                ...
                if tokens:
                    ...
                    # Hapus pesan otomatis (10 detik agar driver sempat copy)
...
def register_token_handlers(dp):
    """Fungsi untuk dipanggil di bot.py"""
    dp.add_handler(CommandHandler("token", token_menu))
    # Gunakan Regex Pattern agar tidak bertabrakan dengan modul lain
