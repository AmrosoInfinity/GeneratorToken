...
def button_handler(update, context):
    ...
    query.answer() # Hilangkan loading icon di Telegram
    ...
    try:
        ...
        if data in ["grab", "gojek"]:
            ...
            # --- EKSEKUSI GOJEK ---
                ...
                # 1. Cek Limit Terlebih Dahulu
                if check_limit(update, context, tz_name, user_id, user_requests, user_blocked, user_timezone):
                    # 2. Tampilkan status loading agar user tidak bingung (karena ada retry logic)
                    query.edit_message_text("📡 *Menghubungkan ke Database Gist...*", parse_mode="Markdown")
                    ...
                    if tokens:
                        ...
                        # 3. Format Pesan (Gunakan Fallback jika string.py bermasalah)
                        try:
                            pesan = string.TOKEN_GOJEK.format(token=f"```{token}```")
                        except:
                            pesan = f"⚗️ **Token Gojek Berhasil Diambil**\n\n```{token}```\n\n_Pesan otomatis hapus dalam 15 detik._"
                        
                        query.edit_message_text(pesan, parse_mode="Markdown")
                        
                        # 4. Simpan Log Penggunaan
                        ...
                        # 5. Timer Hapus Pesan (Beri waktu 15 detik untuk copy-paste)
                        context.job_queue.run_once(
                            lambda ctx: ctx.bot.delete_message(query.message.chat_id, query.message.message_id),
                            15
                        )
                    else:
                        # Jika fetch_tokens mengembalikan list kosong [] setelah retry
                        query.edit_message_text("❌ **Gagal:** Database sedang sibuk atau token tidak ditemukan.\nCoba lagi beberapa saat lagi.", parse_mode="Markdown")

            # Bersihkan owner state setelah selesai
        ...
        logger.error(f"Error di Button Handler: {e}", exc_info=True)
        # Jangan diam saja, beri tahu user jika ada crash internal
        try:
